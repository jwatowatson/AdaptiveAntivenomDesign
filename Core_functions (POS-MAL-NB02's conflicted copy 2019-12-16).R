...
Estimate_log2_Vstar = function(model_params, true_model, MTT, TEL){
  ...
  if(!is.null(model_params) & is.null(true_model)) {
    ...
    TED = model_params$beta_venom * qnorm(p = TEL, mean = model_params$mu_venom, sd = model_params$sd_venom)
...
estimate_posterior_params = function(Trial_data=Trial_data,
  ...
  my_fit = sampling(dose_response_model,
                    data = list(N = nrow(Trial_data),
                                ...
                                mu_venom_mean = prior_model_params$mu_venom_mean,
                                mu_venom_sd = prior_model_params$mu_venom_sd,
                                sd_venom_prior = prior_model_params$sd_venom_prior,
                                beta_venom_mean = prior_model_params$beta_venom_mean,
                                beta_venom_sd = prior_model_params$beta_venom_sd),
                    iter = iter, 
  ...
  posterior_model_params = list(beta_tox = model_params['beta_t','mean'],
                                ...
                                mu_venom = model_params['mu_venom','mean'], 
                                sd_venom = model_params['sd_venom','mean'],
                                beta_venom = model_params['beta_venom','mean'])
